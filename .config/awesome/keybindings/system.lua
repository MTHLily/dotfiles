...
local system = tbl.join({},
    awful.key({keys.meta, keys.shift}, "r", awesome.restart,
        {description = "reload awesome", group = "system"}),
    awful.key({keys.meta}, "x",
...
local input = tbl.join({}, awful.key({keys.meta}, ".", function()
    awful.spawn.with_shell("$HOME/.config/rofi/scripts/emojiselector.sh")
end, {description = "emoji keyboard", group = "system"}))

local sound = tbl.join({}, awful.key({keys.ctrl, keys.alt}, "u", function()
    awful.util.spawn("pavucontrol")
end, {description = "Audio Control", group = "system/sound"}),
...
local screen = tbl.join({}, awful.key({}, "XF86MonBrightnessUp", function()
    ...
    awful.key({keys.meta}, "p", function()
