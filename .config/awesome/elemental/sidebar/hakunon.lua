...
exit:buttons(gears.table.join(awful.button({}, 1, function()
    exit_screen_show()
...
temperature:buttons(gears.table.join(awful.button({}, 1,
    apps.temperature_monitor)))
...
cpu:buttons(gears.table.join(awful.button({}, 1, apps.process_monitor),
...
ram:buttons(gears.table.join(awful.button({}, 1, apps.process_monitor),
    awful.button({}, 3, apps.process_monitor_gui)))

local playerctl_toggle_icon = wibox.widget
                                  .imagebox(icons.image.playerctl_toggle)
...
playerctl_toggle_icon:buttons(gears.table.join(
    awful.button({}, 1, function() awful.spawn.with_shell("mpc -q toggle") end),
    awful.button({}, 3, function() awful.spawn.with_shell("mpvc toggle") end)))
...
playerctl_prev_icon:buttons(gears.table.join(
    awful.button({}, 1, function() awful.spawn.with_shell("mpc -q prev") end),
    awful.button({}, 3, function() awful.spawn.with_shell("mpvc prev") end)))
...
playerctl_next_icon:buttons(gears.table.join(
    awful.button({}, 1, function() awful.spawn.with_shell("mpc -q next") end),
    awful.button({}, 3, function() awful.spawn.with_shell("mpvc next") end)))
...
local fancy_date = wibox.widget.textclock(
    "Knowing that today is %A fills you with determination.")
fancy_date.align = "center"
fancy_date.valign = "center"
fancy_date.font = "sans italic 11"

local spotify_widget = require("noodle.spotify")
...
mpd_song:buttons(gears.table.join(awful.button({}, 1, function()
    awful.spawn.with_shell("mpc -q toggle")
end), awful.button({}, 3, function()
    -- Spawn music terminal
    awful.spawn(user.music_client)
end), awful.button({}, 4, function() awful.spawn.with_shell("mpc -q prev") end),
    awful.button({}, 5, function() awful.spawn.with_shell("mpc -q next") end)))
...
local disk = wibox.widget {
    nil,
    {disk_icon, disk_space, layout = wibox.layout.fixed.horizontal},
    nil,
    expand = "none",
    layout = wibox.layout.align.horizontal
}
...
awful.button({}, 1, function() helpers.volume_control(0) end),
    ...
    awful.button({}, 3, apps.volume), -- Scroll - Increase / Decrease volume
...
sidebar_toggle = function() sidebar.visible = not sidebar.visible end

sidebar:buttons(gears.table.join( -- Middle click - Hide sidebar
awful.button({}, 2, function() sidebar.visible = false end)))

-- Hide sidebar when mouse leaves
if user.sidebar.hide_on_mouse_leave then
    sidebar:connect_signal("mouse::leave",
        function() sidebar.visible = false end)
end
