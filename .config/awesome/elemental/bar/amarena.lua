...
local dock_placement = function(w) return awful.placement.bottom(w) end
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.taglist_box:setup{widget = s.mytaglist}
    ...
    s.dock:connect_signal("mouse::leave", function() autohide() end)
    s.dock_activator:connect_signal("mouse::leave", function() autohide() end)

    -- Create a system tray widget
    s.systray = wibox.widget.systray()
    s.systray.base_size = dpi(20)
    -- Create the tray box
    s.traybox = wibox({
        screen = s,
        width = dpi(150) * 2,
